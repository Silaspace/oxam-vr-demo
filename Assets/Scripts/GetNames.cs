...
public class GetNames : MonoBehaviour
{
    ...
    public static (List<Vector3>, List<string>, List<string>) fetch(string inputfile)
    {
        ...
        {   
            ...
            namesList.Add("x " + Convert.ToString(pointList[i][xAxisKey]) + " y "
                + Convert.ToString(pointList[i][yAxisKey]) + " z "
                + Convert.ToString(pointList[i][zAxisKey]));
        }
        ...
        return (vectorList, labels, namesList);
    }
